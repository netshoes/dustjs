(function (root, factory) {
  ...
  Stub.prototype.flush = function(isDelayedChunk) {
    var chunk = this.head;
    var idx;
    ...
    while (chunk) {
      ...
      if (chunk.next && chunk.next.delayed) {
        this.out.push(null);
        this.delayedChunks.push(chunk.next);
      }

      // Delayed API
      if (isDelayedChunk) {
        idx = this.out.indexOf(null);

        if (idx >= 0) {
          this.out[idx] = chunk.data.join('');
        }

        if (chunk.next.delayed) {
          chunk = chunk.next;
          this.out.splice(idx,0,null);
        } else {
          chunk = null;
        }
      } else {
        chunk = chunk.getNonDelayedNext();
      }

      this.head = chunk;
    }

    if (this.delayedChunks.length) {
      this.head = this.delayedChunks.shift();
      this.head.flushable = true;
      this.flush(true);
      return;
    }

    this.callback(null, this.out.join(''));
  };
  ...
  Stream.prototype.flush = function() {
    ...
    while(chunk) {
      if (chunk.flushable) {
        ...
        // TODO: make delayed chunks compatible with streams
        if (chunk.delayed) {
          chunk.end();
        }
        return;
  ...
  Chunk.prototype.getNonDelayedNext = function() {
    if (!this.next) {
      return null;
    }

    if (this.next.delayed) {
      return this.next.getNonDelayedNext();
    }
    return this.next;
  };
